...
1. **Accept Request:** If the Proposer receives `Promise` responses from a quorum, it proceeds.
...
2. **Accepted:** An Acceptor receives `Accept(N, V)`. It will accept the value $V$ **unless** it has already responded to another Proposer's `Prepare` request with a number greater than $N$. If accepted, it broadcasts an `Accepted` message to all Learners.
