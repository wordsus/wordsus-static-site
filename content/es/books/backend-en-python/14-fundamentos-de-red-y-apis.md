Has cruzado el umbral de la ejecución local para entrar en el ecosistema de los sistemas distribuidos. En este capítulo, exploraremos el tejido que conecta al mundo: el protocolo HTTP/HTTPS, base de cualquier API moderna. No nos limitaremos a realizar peticiones; diseccionaremos la semántica de sus métodos, la importancia de los códigos de estado y el rol crítico de las cabeceras. Además, compararemos el estándar REST con alternativas de alto rendimiento como GraphQL y gRPC, para cerrar dominando la comunicación en tiempo real mediante WebSockets. Aquí es donde transformas tus scripts de Python en servicios capaces de sostener la arquitectura de la internet actual.
