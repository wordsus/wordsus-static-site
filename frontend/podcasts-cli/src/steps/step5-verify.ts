/**
 * Step 5 — Verify that all sources are ready in sources-today/.
...
export async function runStep5(session: SessionState): Promise<void> {
  ...
  while (true) {
    ...
    if (allReady) {
      ok("All source files are present in sources-today/.");
      ...
      console.log(C.white("  and placed them in the sources-today/ directory with the correct alias as filename."));
