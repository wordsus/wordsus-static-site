...
const MENU_ITEMS = [
  { key: "1", label: "Create JSON Metadata files",      value: 1 },
  { key: "2", label: "Copy Article URLs (NotebookLM)",  value: 2 },
  { key: "3", label: "Copy Audio Prompts (NotebookLM)", value: 3 },
  { key: "4", label: "Copy Image Prompts (Gemini)",     value: 4 },
  { key: "5", label: "Verify Source Files readiness",   value: 5 },
  { key: "6", label: "Generate Videos (FFmpeg)",        value: 6 },
  { key: "7", label: "Backup & Cleanup",                value: 7 },
  { key: "0", label: "Exit program",                    value: 0 },
];

// ── Menu renderer ─────────────────────────────────────────────────────────────

function renderMenu(defaultEpisode: number, selectedIdx: number): void {
  printBanner();
  console.log();
  console.log(C.muted("  Default episode: ") + C.accent.bold(String(defaultEpisode)));
  console.log();
  ...
  console.log();
  console.log(C.muted("  ↑↓ navigate  ·  enter select  ·  0–7 shortcut  ·  ⌫ clear screen"));
}
 ...
 *   0–7            — jump to item and confirm after 120 ms (visual flash)
 *   Backspace/DEL  — resolve with "clear"
 *   Ctrl+C         — exit process
 */
function showMenu(defaultEpisode: number, lastChoice: number): Promise<number | "clear"> {
  return new Promise((resolve) => {
    ...
    renderMenu(defaultEpisode, selectedIdx);
    ...
    const onData = (data: Buffer) => {
      ...
      // ── Digit shortcut 0–7: jump to item, flash highlight, then confirm ─
      if (data[0] >= 0x30 && data[0] <= 0x37) {
...
async function main() {
  ...
  if (epIndex !== -1 && args.length > epIndex + 1) {
    ...
    if (!isNaN(num)) defaultEpisodeArg = num;
  }
  ...
  try {
    ...
    while (!exitRequested) {
      const choice = await showMenu(session.defaultEpisode, lastChoice);
      ...
      try {
        switch (choice) {
          ...
          case 6: await runStep6(session); break;
          case 7: await runStep7(); break;
